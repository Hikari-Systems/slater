...
| `unsupported write: the writable layer accepts business-key MERGE / SET / REMOVE / [DETACH] DELETE, CREATE / INSERT (GQL), and relationship writes only` | The statement's shape isn't one the writable layer accepts. | Use a supported write shape ([11 Writing data](11-writing-data.md)). A `RETURN` after a write is fine — that used to be the common cause of this and no longer is. |
...
| `the writable layer is bound to generation X but graph 'g' is now serving Y … has NOT been applied` | A new generation was swapped in under the writer, so the delta's internal ids no longer line up with what is served. | The write did **not** land — retry it after consolidating or restarting so the delta rebinds. Publishing under a live writable server is what causes this. |
| `refusing to swap graph 'g' to generation X: the writable layer holds N pending entities` | A new generation appeared on disk while the delta held unconsolidated writes. The guard kept the current one serving rather than abandoning them. | `CALL slater.consolidate()` to fold the delta in, then publish again. Not an error in the generation — it is still there. |
| `refusing to consolidate 'g': … hashes to <b> but the ACL in force is <a>` | `acl.json` changed since the server adopted it, so a rebuild would be stamped against a file the server never accepted. | Restore the file, or restart so the new ACL is adopted through the stamp gate, then consolidate. |
| `refusing the consolidated generation of 'g': it is stamped … but the ACL in force is …` | The rebuild read an `acl.json` that changed while it ran. | Same remedy. The `current` pointer is rolled back to the generation still serving, so the graph stays bootable. |
